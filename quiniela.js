...
// CON FIREBASE, 5 LOGOS CENTRADOS Y PUNTOS SEMANALES / POR FASE
// ============================================
...
async function cargarParticipantesFirebase() {
    try {
        ...
        snapshot.forEach(doc => {
            const data = doc.data();
            // Inicializar campos para compatibilidad
            if (!data.puntosPorSemana) data.puntosPorSemana = {};
            if (!data.puntosPorFase) data.puntosPorFase = {};
...
// ------------------- FUNCIONES PARA MANEJO DE SEMANAS Y FASES -------------------
...
async function registrarParticipante(nombre, cedula) {
    ...
    try {
        await db.collection('quiniela_participantes').doc(cedulaNormalizada).set(nuevoParticipante);
        console.log("✅ Participante guardado en Firebase");
    } catch (error) {
        console.error("Error guardando participante:", error);
    ...
    if (storedUsuario) {
        try {
            ...
            if (parsed && parsed.cedula) {
                ...
                // Verificar que el usuario exista en Firebase (opcional)
                actualizarUIUsuario();
                mostrarPartidos();
            } else {
                localStorage.removeItem('quiniela_usuario_actual');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('semanaSelector')?.addEventListener('change', mostrarRanking);
    
    setTimeout(() => agregarBotonAdmin(), 500);
});

// Exponer funciones globales para uso en HTML (onclick)
